...
def usb_root() -> Optional[str]:
    d = app_dir()
    if os.path.basename(d).lower() in ("pixelflap", "pixel-flap", "pixel flap"):
...
class PixelFlapGame:
    def __init__(self) -> None:
        pygame.init()
        pygame.display.set_caption("Pixel Flap — UsbGames")
    ...
    def _spawn_pipe(self, x: float) -> None:
        ...
        self.pipes.append(Pipe(x=x, gap_y=gy, gap_h=gap))
    ...
    def update(self, dt: float) -> None:
        ...
        self.elapsed += dt
        self.pipe_speed = min(MAX_SPEED, BASE_SPEED + self.elapsed * 2.2)
    ...
    def _draw_title(self) -> None:
        ...
        self._pixel_text("PIXEL FLAP", WIN_W // 2, 100, self.font_lg, COL_TURQ, center=True)
